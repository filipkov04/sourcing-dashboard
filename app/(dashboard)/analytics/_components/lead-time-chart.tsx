        ...
        {activeView === "chart" ? (
          <ResponsiveContainer width="100%" height={data.length * 44 + 30}>
            <BarChart data={data} layout="vertical" margin={{ top: 5, right: 40, left: 0, bottom: 5 }}>
              <defs>
                <marker id="arrow-x-lt" markerWidth="10" markerHeight="10" refX="10" refY="5">
                  <path d="M0,0 L10,5 L0,10 Z" fill={isDark ? "#3f3f46" : "#e5e7eb"} />
                </marker>
              </defs>
              <XAxis type="number" tick={{ fontSize: 11, fill: isDark ? "#71717a" : "#9ca3af" }} tickLine={{ stroke: isDark ? "#3f3f46" : "#e5e7eb", strokeWidth: 1 }} tickSize={4} axisLine={{ stroke: isDark ? "#3f3f46" : "#e5e7eb", strokeWidth: 1, markerEnd: "url(#arrow-x-lt)" }} unit="d" padding={{ right: 15 }} />
              <YAxis type="category" dataKey="factoryName" width={120} tick={{ fontSize: 12, fill: isDark ? "#a1a1aa" : "#6b7280" }} tickLine={false} axisLine={{ stroke: isDark ? "#3f3f46" : "#e5e7eb", strokeWidth: 1 }} />
              <Tooltip
